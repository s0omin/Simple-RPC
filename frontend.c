...
/*
        ...
        while (waitpid(pid, &rval, WNOHANG) == 0) {
            if ((id = fork()) == 0) {
                ...
                if (waitpid(pid, &rval, WNOHANG)) { // if parent receives signal from a child process to notify quit/shutdown
                    done = 1;
                    break;
                }
